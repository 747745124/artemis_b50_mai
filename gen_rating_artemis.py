...
display_name = "YourUsername" # this is the name to be displayed on the b50
...
mycursor.execute(
    '''
SELECT songId, title, chartId, difficulty, comboStatus
...
GROUP BY songId, title, chartId, difficulty, comboStatus 
''')
...
def convert_to_maibot_data(rating_list, isDx = True):
    for i in range(len(rating_list)):
        ...
        song_id = str(song_info[0]).zfill(5)
        ...
        data={"id":song_id,"title":song_name,"level_index":difficulty,"ds":internal_lv,
        "achievements":achv,"rate":rate,"fc":combo_status,"level":"","type":""}
