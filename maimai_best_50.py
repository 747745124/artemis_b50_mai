...
def local_generate50(json_obj, username) -> Tuple[Optional[Image.Image], bool]:
    sd_best = BestList(35)
    dx_best = BestList(15)
    ...
    for c in sd:
        sd_best.push(ChartInfo.from_json_local(c))
    for c in dx:
        dx_best.push(ChartInfo.from_json_local(c))

    pic = DrawBest(sd_best, dx_best, username).getDir()
    return pic, 0
